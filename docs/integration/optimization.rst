...
and a pymoo algorithm into a single call and returns a tuple of a
:code:`DataFrame` log and the pymoo result object. For more information on
...
The :code:`DataFrame` log contains every evaluated individual, including
infeasible ones. When constraints are active, filter for feasibility before
selecting the optimum - the plotting code in the download demonstrates this.
Alternatively, the pymoo result object (:code:`result.X`, :code:`result.F`)
provides only feasible solutions directly and requires no further filtering.
