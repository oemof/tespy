                ...
                if f"{key}_ref" in self.property_data:
                    if self.get_attr(f"{key}_ref" ).is_set:
                        msg = (
                            ...
                            "will automatically unset a specified Ref "
            ...
            if f"{key}_ref" not in self.property_data:
                ...
                if self.get_attr(key).is_set:
                    msg = (
                        ...
                        f".set_attr({key}=Ref(...))."
                        "With the next major release of TESPy setting a Ref "
                        "will automatically unset a specified numerical "
                        "value making it impossible to set a numerical value "
                        "and a Ref for one parameter on one connection "
                        "simultaneously."
                    )
                    warnings.warn(msg, FutureWarning)
