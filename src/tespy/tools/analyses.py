...
class ExergyAnalysis:
    ...
    def __init__(self, network, E_F, E_P, E_L=[], internal_busses=[]):
        ...
        Start by defining ambient state and general network setup.
        ...
        this bus is specified to be 0 in order to make sure, the power
        generated by the secondary steam turbine is transferred to the feed
        water pump. For mechanical efficiency we choose 0.985 for both
        components, but we need to make sure, the :code:`'base'` of the feed
        water pump is :code:`'bus'` as the energy from the turbine drives the
        feed water pump.
        ...
        The method returns a dictionary containing the links for the sankey
