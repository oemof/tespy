...
class ThermopackWrapper(FluidPropertyWrapper):
    ...
    def T_ps(self, p, s):
        return self.AS.two_phase_psflash(p, self._z, s * self._molar_mass).T

    def h_pT(self, p, T):
        return self._h_flash(self.AS.two_phase_tpflash(T, p, self._z))

    def h_ps(self, p, s):
        return self._h_flash(
            self.AS.two_phase_psflash(p, self._z, s * self._molar_mass)
        )
    ...
    def d_pT(self, p, T):
        return self._d_flash(self.AS.two_phase_tpflash(T, p, self._z))

    def s_ph(self, p, h):
        return self._s_flash(self._update_ph(p, h))

    def s_pT(self, p, T):
        return self._s_flash(self.AS.two_phase_tpflash(T, p, self._z))
